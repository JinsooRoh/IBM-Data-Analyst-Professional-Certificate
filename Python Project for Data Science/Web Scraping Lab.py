...
# In this lab, we will cover BeautifulSoup and Tag objects that for the purposes of this lab are identical, and NavigableString objects.
...
# Beautiful Soup uses the NavigableString class to contain this text. In our HTML we can obtain the name of the first player by extracting the sting of the Tag object tag_child as follows:
...
# For example the id argument, Beautiful Soup will filter against each tag’s id attribute.
# For example, the first td elements have a value of id of flight, therefore we can filter based on that id value.
result = table_bs.find_all(id="flight")
print(result)
...
table1 = two_tables_bs.find("table")
print(table1)
...
# Open the above url in your browser and check how many rows and columns are there in the color table.
...
# Before proceeding to scrape a web site, you need to examine the contents, and the way data is organized on the website.
# Open the above url in your browser and check the tables on the webpage.
...
# Remember the table we need is located in
tables[table_index]
